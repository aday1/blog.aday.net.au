...
const deployMetaHtml = `<div id="deployMetaPanel" style="position:fixed;right:10px;bottom:10px;z-index:9999;max-width:min(400px,calc(100vw - 20px));font:11px/1.45 ui-monospace,Consolas,monospace;border:1px solid #4cff8a;background:rgba(0,0,0,.92);color:#b8ffd1;padding:10px 12px;box-sizing:border-box;border-radius:3px;box-shadow:0 4px 18px rgba(0,0,0,.55);">
  ...
  <script>
    (function () {
      ...
      var wrap = document.getElementById("deployMetaPanel");
      ...
      function age(iso) {
        ...
        if (sec < 4233600) return Math.round(sec / 604800) + " wk ago";
        var mo = Math.round(sec / (30.4375 * 86400));
        if (mo < 24) return mo + " mo ago";
        return Math.round(sec / (365.2425 * 86400)) + " yr ago";
      }
      function render(m) {
        var sha = esc(m.last_git_sha_short || m.version || "?");
        ...
        if (ageB) html += " <span style=\\"opacity:.82\\"><strong>Age:</strong> " + esc(ageB) + "</span>";
        html += "</div>";
        html += "<div style=\\"margin:4px 0\\"><strong>Last deployed (CI push):</strong> " + esc(fmt(depIso));
        if (ageD) html += " <span style=\\"opacity:.82\\"><strong>Age:</strong> " + esc(ageD) + "</span>";
